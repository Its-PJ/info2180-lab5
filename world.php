...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ( isset($_GET['country']) && !empty($_GET['country']) && !isset($_GET['context']) ) {
        $countrySearch = ucwords(trim(strip_tags($_GET["country"])));
   ...
   <?php  }
   else if ( isset($_GET['country']) && !empty($_GET['country']) && isset($_GET['context']) && $_GET['context'] == "allcities" ){
        $countrySearch = ucwords(trim(filter_var($_GET['country'], FILTER_SANITIZE_STRING)));
        $stmt = $conn->query("SELECT cities.name, cities.district, cities.population FROM cities  JOIN 
        countries ON cities.country_code = countries.code  WHERE countries.name LIKE '%$countrySearch%' ");
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);?>
